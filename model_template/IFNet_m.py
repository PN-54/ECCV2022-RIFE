...
from model_template.refine import *
...
class IFNet_m(nn.Module):
    def __init__(self, b1c=240, b2c=150, b3c=90, cc=16):
        # RIFE Milli: b1c=70, b2c=44, b3c=26, cc=1
        # RIFE Micro: b1c=156, b2c=0, b3c=0, cc=0
        ...
        self.num_blocks = 0
        if b1c > 0:
            self.block0 = IFBlock(6+1, c=b1c)
            self.num_blocks += 1
        if b2c > 0:
            self.block1 = IFBlock(13+4+1, c=b2c)
            self.num_blocks += 1
        if b3c > 0:
            self.block2 = IFBlock(13+4+1, c=b3c)
            self.num_blocks += 1
        self.block_tea = IFBlock(16+4+1, c=90)
        if cc > 0:
            self.contextnet = Contextnet(cc=cc)
        if cc >= 0:
            self.unet = Unet(cc=cc)
    ...
    def forward(self, x, scale=[4,2,1], timestep=0.5, returnflow=False):
        ...
        if returnflow:
            ...
            final = self.num_blocks - 1
            if self.cc > 0:
                c0 = self.contextnet(img0, flow[:, :2])
                c1 = self.contextnet(img1, flow[:, 2:4])
                tmp = self.unet(img0, img1, warped_img0, warped_img1, mask, flow, c0, c1)
            elif self.cc == 0:
                tmp = self.unet(img0, img1, warped_img0, warped_img1, mask, flow, None, None)
                res = tmp[:, :3] * 2 - 1
                merged[final] = torch.clamp(merged[final] + res, 0, 1)
            else:
                merged[final] = torch.clamp(merged[final], 0, 1)
        return flow_list, mask_list[final], merged, flow_teacher, merged_teacher, loss_distill
